...
public final class TiltGlassPrefs {
    ...
    public void calibrate(float x, float y, float z) {
        preferences.edit().putFloat("x", x).putFloat("y", y).putFloat("z", z).putBoolean("calibrated", true).remove("orientation_calibrated").apply();
    }
    ...
    public void calibrateOrientation(float[] matrix) {
        preferences.edit().putFloat("x",matrix[6]*9.81f).putFloat("y",matrix[7]*9.81f)
    ...
    public void resetReference() {
        preferences.edit().remove("x").remove("y").remove("z").remove("calibrated").remove("nx").remove("ny").remove("nz").remove("orientation_calibrated").apply();
    }
}
